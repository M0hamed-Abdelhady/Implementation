...
class SegmentTree {
    ...
    void propagate(int L, int R, int node) {
        if (lazy[node]) {
            tree[node] += (ll) lazy[node] * ( R- L + 1);
            if (L != R)
                lazy[left(node)] += lazy[node], lazy[right(node)] += lazy[node];
            lazy[node] = 0;
        }
    }
    ...
    void build(vector<ll> &x) {
        ...
        tree.assign(n << 1, opposite), lazy.assign(n << 1, 0);
    ...
    void update(int L, int R, int node, int Lq, int Rq, int value) {
        ...
        if (L != R) tree[node] = cmp(tree[left(node)], tree[right(node)]);
    }

    ll RSQ(int L, int R, int node, int Lq, int Rq) {
        propagate(L, R, node);
        if (L == Lq && R == Rq) return tree[node];
        if (Lq > R || Rq < L)return opposite;
        int mid = L + R >> 1;
        return cmp(RSQ(L, mid, left(node), Lq, min(mid, Rq)),
                   RSQ(mid + 1, R, right(node), max(Lq, mid + 1), Rq));
    }
...
public:
    ...
    ll RSQ(int L, int R) { return RSQ(0, n - 1, 1, L, R); }

    ll RMQ(int i) { return RSQ(0, n - 1, 1, i, i); }
};
...
int main() {
    ...
    SegmentTree tree(a);
    tree.update(0, 2, 120);
//  update range from L to R (Add to them)
    tree.update(4, 150);
//  update index i (Add to it)
    cout << tree.RSQ(0, 3) << '\n';
    cout << tree.RSQ(2, 4) << '\n';
//  Range Maximum Query
}